...
import { ArrowLeft, ArrowRight, Check, Close, Delete, EditPen, PictureFilled, Upload } from '@element-plus/icons-vue'
...
const imageThumbnailsVisible = ref(true)
...
function previewPrev() {
  ...
  imagePreviewIndex.value = (imagePreviewIndex.value - 1 + mediaItems.value.length) % mediaItems.value.length
}

function previewNext() {
  if (mediaItems.value.length <= 1) return
  imagePreviewIndex.value = (imagePreviewIndex.value + 1) % mediaItems.value.length
}
...
<template>
  <div class="subpage-shell">
    ...
    <main class="subpage-main action-form-page" v-loading="loading">
      <section v-if="pet" class="action-form-panel pet-media-upload-panel">
        ...
        <div v-if="mediaItems.length" class="pet-album-grid">
          <article v-for="item in mediaItems" :key="item.id" class="pet-album-card" @click="openMediaPreview(item)">
            ...
            <div class="pet-album-card-body">
              <div>
                <strong><el-tooltip :content="item.name || ''" :disabled="!item.name" placement="top">{{ item.name || '未命名媒体' }}</el-tooltip></strong>
                <span>{{ item.type === 'IMAGE' ? '图片' : '视频' }} · {{ formatDate(item.createDate) }}</span>
              </div>
              <p v-if="item.description">{{ item.description }}</p>
              <div v-if="canEditCurrentPet" class="pet-album-card-actions" @click.stop>
    ...
    <Transition name="viewer-fade">
      <div v-if="imagePreviewVisible" class="image-viewer-overlay" :class="{ 'image-viewer-thumbs-hidden': !imageThumbnailsVisible || mediaItems.length <= 1 }" @click.self="closeImagePreview">
        <button class="image-viewer-close" @click="closeImagePreview"><el-icon><Close /></el-icon></button>
        ...
        <div class="image-viewer-main">
          <img v-if="isImage(mediaItems[imagePreviewIndex])" :src="mediaItems[imagePreviewIndex]?.assetUrl" :alt="mediaItems[imagePreviewIndex]?.name" />
          <video v-else :src="mediaItems[imagePreviewIndex]?.assetUrl" controls autoplay class="image-viewer-video" />
        </div>

        <Transition name="thumb-slide">
          <div v-if="imageThumbnailsVisible && mediaItems.length > 1" class="image-viewer-thumbs">
            <div
              v-for="(thumb, idx) in mediaItems"
              :key="thumb.id"
              class="image-viewer-thumb-item"
              :class="{ 'is-active': idx === imagePreviewIndex }"
              @click.stop="imagePreviewIndex = idx"
            >
        ...
        <button class="image-viewer-thumb-toggle" @click.stop="imageThumbnailsVisible = !imageThumbnailsVisible">
